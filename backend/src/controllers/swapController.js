  ...
  try {
    ...
    if (!offeredSkill || !requestedSkill || !proposerDeadline) {
      return res
        ...
        .json({
          message:
            "offeredSkill, requestedSkill, and proposerDeadline are required",
        });
    }
    ...
    const userSkills = Array.isArray(user.skillsOffered)
      ? user.skillsOffered
      : [];
    const offeredSkillsArray = Array.isArray(offeredSkill)
      ? offeredSkill
      : [offeredSkill];
    const invalidSkills = offeredSkillsArray.filter(
    ...
    if (invalidSkills.length > 0) {
      return res
        .status(400)
        .json({
          message: `You can only offer your own skills. Invalid: ${invalidSkills.join(
            ", "
          )}`,
        });
    }
  ...
  try {
    ...
    if (status === "accepted") {
      if (swap.status !== "pending" || swap.receiver) {
        return res
          .status(400)
          .json({ message: "Swap already accepted or not open" });
      }
      ...
      if (!deadline) {
        return res
          .status(400)
          .json({ message: "Deadline is required when accepting a swap" });
      }
      if (!acceptorMessage || !acceptorMessage.trim()) {
        return res
          .status(400)
          .json({ message: "Please provide a description of what you need" });
      }
    ...
    if (status === "task_completed") {
      ...
      if (swap[`${otherRole}TaskCompleted`]) {
        ...
        swap.status =
          userRole === "sender" ? "sender_completed" : "receiver_completed";
      }
    ...
    if (approval === "approve") {
      ...
      const isReceiver =
        swap.receiver && swap.receiver.toString() === req.user.id;
      ...
      if (!swap[`${otherRole}TaskCompleted`]) {
        return res
          .status(400)
          .json({
            message: "Cannot approve before other user completes their task",
          });
      }
      ...
      if (swap[`${userRole}Approved`]) {
        ...
        try {
          ...
          console.error(
            "Error updating user stats after swap completion:",
            err
          );
    ...
    if (
      status === "completed" &&
      ![swap.sender.toString(), swap.receiver?.toString()].includes(req.user.id)
    ) {
      return res.status(403).json({ message: "Not authorized" });
    }
